...
if len(sys.argv) > 1:
    ...
    sys.exit("Say what you want to plot.")
...
# Load the approximate spacing of the nodes
dr = np.array([], float)
...
xtmp1 = radius * np.cos(th)
ytmp1 = radius * np.sin(th)
xtmp2 = xtmp1 - (2*radius + 3*dr)
ytmp2 = ytmp1
xtmp3 = xtmp1 + (2*radius +3*dr)
ytmp3 = ytmp1
...
plt.plot(x, y, 'k.', \
         ...
         x[indB], y[indB], 'gs', \
         x[indC], y[indC], 'ys', \
         # (x[indB]+x[indC])/2, (y[indB]+y[indC])/2, 'k*', \
...
doTheTimeStepping = ""
with open(instr + 'doTheTimeStepping.txt') as f:
    for line in f:
        doTheTimeStepping = doTheTimeStepping + line

if (doTheTimeStepping[:-1] == "true"):
    ...
    while True:
        ...
        try:
            ...
            fig.colorbar(cs)
            plt.axis('equal')
            plt.axis(33 * np.array([-1,1,-1/3,1/3]))
    
            plt.plot(xtmp1, ytmp1, 'k', xtmp2, ytmp2, 'k', xtmp3, ytmp3, 'k')
